...
管理员用例图的绘制结果见图3-5。管理员登录进入本系统操作的功能包括管理宿舍信息，管理学生评分信息，管理宿舍评分信息，管理来访登记信息，管理维修登记信息，管理班级与学生信息等。

![](/md/blog.005.png)

图3-5 管理员用例图

学生用例图的绘制结果见图3-6。学生登录进入本系统操作的功能包括查看学生评分，查看宿舍评分信息，查看宿舍信息，登记宿舍维修信息，管理宿舍维修信息，查看公告等。
...
管理员功能结构图的绘制结果见图4-1。管理员登录进入本系统操作的功能包括管理宿舍信息，管理学生评分信息，管理宿舍评分信息，管理来访登记信息，管理维修登记信息，管理班级与学生信息等。

![](/md/blog.007.png)

图4-1 管理员功能结构图

学生功能结构图的绘制结果见图4-2。学生登录进入本系统操作的功能包括查看学生评分，查看宿舍评分信息，查看宿舍信息，登记宿舍维修信息，管理宿舍维修信息，查看公告等。
...
（3）使用Visio这样的常用的实体属性图绘制工具来绘制宿舍评分实体属性图，绘制结果见图4-5。

![](/md/blog.011.png)

图4-5 宿舍评分实体属性图

（4）使用Visio这样的常用的实体属性图绘制工具来绘制宿舍信息实体属性图，绘制结果见图4-6。
...
通过上节内容可以知晓数据库中的各个实体，并通过一定方式把这些实体表示的内容进行数据表的转换，通常来说，每个实体都会对应一张具体的数据表，在本系统指定的数据库中创建命名好的数据库，才可以对数据表进行创建与设计。宿舍管理系统数据表设计结果展示如下：
...
|banjihao|班级号|varchar(200)|是|
|banjimingcheng|班级名称|varchar(200)|是|
|banjirenshu|班级人数|int(11)|是|
|fudaoyuan|辅导员|varchar(200)|是|
|xueshengmingdan|学生名单|longtext|是|
表4-2 来访登记表
...
|id (主键)|主键|bigint(20)|否|
|addtime|创建时间|timestamp|否|
|laifangbianhao|来访编号|varchar(200)|是|
|fangkexingming|访客姓名|varchar(200)|是|
|laifangrenshu|来访人数|int(11)|是|
|shenfenzheng|身份证|varchar(200)|是|
|laifangxuesheng|来访学生|varchar(200)|是|
|xueshengxingming|学生姓名|varchar(200)|是|
|sushehao|宿舍号|varchar(200)|是|
|laifangmude|来访目的|longtext|是|
|laifangshijian|来访时间|datetime|是|
表4-3 公告信息表
...
|xueshengxingming|学生姓名|varchar(200)|是|
|sushehao|宿舍号|varchar(200)|是|
|shifouzhengjie|是否整洁|varchar(200)|是|
|shifouweigui|是否违规|varchar(200)|是|
|jianchaneirong|检查内容|varchar(200)|是|
|weishengpingfen|卫生评分|int(11)|是|
|jianchajieguo|检查结果|varchar(200)|是|
|jianchariqi|检查日期|date|是|
表4-5 宿舍信息表
...
|id (主键)|主键|bigint(20)|否|
|addtime|创建时间|timestamp|否|
|sushehao|宿舍号|varchar(200)|否|
|tupian|图片|varchar(200)|是|
|susheleixing|宿舍类型|varchar(200)|否|
|loudong|楼栋|varchar(200)|是|
|louceng|楼层|varchar(200)|是|
|xuehao|学号|varchar(200)|是|
|xueshengxingming|学生姓名|varchar(200)|是|
|sushechengyuan|宿舍成员|longtext|是|
表4-6 管理员表
...
|loudong|楼栋|varchar(200)|是|
|louceng|楼层|varchar(200)|是|
|weixiubiaoti|维修标题|varchar(200)|是|
|weixiuneirong|维修内容|longtext|是|
|baoxiushijian|报修时间|date|是|
|xuehao|学号|varchar(200)|是|
表4-8 学生表
...
|yuanxi|院系|varchar(200)|是|
|banjimingcheng|班级名称|varchar(200)|是|
|qingjieweisheng|清洁卫生|int(11)|是|
|wenmingjuzhi|文明举止|int(11)|是|
|zongdefen|总得分|int(11)|是|
|pingyu|评语|longtext|是|
|pingfenshijian|评分时间|date|是|
...
## 5.1管理员功能实现
### 5.1.1 宿舍信息管理
管理员进入如图5-1所示的宿舍信息管理界面之后，管理员点击信息显示栏中最右侧的宿舍评分，修改，删除按钮可依次完成宿舍信息的宿舍评分查看，修改，删除操作。

![](/md/blog.014.png)

图5-1 宿舍信息管理界面
### 5.1.2 来访登记管理
管理员进入如图5-2所示的来访登记管理界面之后，管理员点击信息显示栏中最右侧的修改，删除按钮可依次完成来访登记信息的修改，删除等操作，管理员在当前界面也能添加来访登记信息，查询来访登记信息。

![](/md/blog.015.png)

图5-2 来访登记管理界面
### 5.1.3 学生评分管理
管理员进入如图5-3所示的学生评分管理界面之后，管理员点击信息显示栏右侧的修改，删除按钮可依次完成学生评分信息的修改，删除等操作。学生评分信息包括清洁卫生，文明举止，总得分以及评分时间等信息。

![](/md/blog.016.png)

图5-3 学生评分管理界面
### 5.1.4 宿舍评分管理
管理员进入如图5-4所示的宿舍评分管理界面之后，管理员点击信息显示栏中最右侧的修改，删除按钮可依次完成宿舍评分信息的修改，删除等操作。在宿舍评分管理界面，管理员点击统计按钮可以查看宿舍评分的统计信息，管理员可以根据检查结果，宿舍号来查询宿舍评分信息。
...
## 5.2学生功能实现
### 5.2.1 宿舍信息管理
学生进入如图5-5所示的宿舍信息管理界面之后，学生可以点击信息显示栏右侧的维修登记按钮可以登记宿舍的维修信息，可以在宿舍信息管理界面查询，查看宿舍信息等。

![](/md/blog.018.png)

图5-5 宿舍信息管理界面
### 5.2.2 维修登记管理
学生进入如图5-6所示的维修登记管理界面之后，学生可以查询维修登记信息，查询时，需要提供宿舍号或学号才能查询维修登记信息，在维修登记管理界面，学生也能删除维修登记信息。

![](/md/blog.019.png)

图5-6 维修登记管理界面
### 5.2.3 公告信息查看
学生进入如图5-7所示的公告信息查看界面之后，学生查看公告详细内容，当公告信息比较多时，学生可以使用查询功能来查询公告，查询前需要在查询框中登记标题才可以实现对公告信息的查询。
